...
export class CreateCompanyUseCase {
	...
	public async execute(
		props: CreateCompanyUseCaseRequest,
	): Promise<CreateCompanyUseCaseResponse> {
		const companyAlreadyExists =
			await this.companyRepository.findByEmailOrTaxId(
				props.email,
				props.taxId,
			)

		if (companyAlreadyExists?.taxId === props.taxId) {
			return left(new CompanyTaxIdAlreadyExistsError())
		}

		if (companyAlreadyExists?.email === props.email) {
