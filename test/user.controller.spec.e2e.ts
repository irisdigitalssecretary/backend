...
describe('UserController (E2E)', () => {
	...
	describe('Security / Auth', () => {
		it('should not be performed if authentication does not exist', async () => {
			const newUser = {
				name: 'John Doe',
	...
	describe('PATCH /users/:id/session-status', () => {
		it('should be able to update user session status', async () => {
			const userToCreate = {
				name: 'John Doe',
				email: 'john.doe@example.com',
				password: 'Test123@emaiiil',
		...
		it('should not be able to update user session status if the user does not exist', async () => {
			const newData = {
				status: 'online',
			}
	...
	describe('PATCH /users/:id/status', () => {
		...
		it('should not be able to update user status if the user does not exist', async () => {
			const newData = {
				status: 'inactive',
			}
	...
	describe('DELETE /users/:id', () => {
		...
		it('should not be able to delete a user if the user does not exist', async () => {
			const response = await request(server)
				.delete(`/users/123`)
				.set('Authorization', `Bearer ${env.MASTER_LOCAL_TESTS_KEY}`)
				.set('x-company-id', company.id)
	...
	describe('GET /users/:uuid', () => {
		...
		it('should not be able to get a user by uuid if the user does not exist', async () => {
			const response = await request(server)
				.get(`/users/123`)
				.set('Authorization', `Bearer ${env.MASTER_LOCAL_TESTS_KEY}`)
				.set('x-company-id', company.id)
	...
	describe('GET /users', () => {
		...
		it('should be able to find many users with custom pagination', async () => {
			for (let i = 1; i <= 5; i++) {
				await createTestUser(company.id, server, i)
			}
			const response = await request(server)
		...
		it('should respect max limit of 70', async () => {
			for (let i = 1; i <= 80; i++) {
				await request(server)
					.post('/users')
					.set('Authorization', `Bearer ${env.MASTER_LOCAL_TESTS_KEY}`)
		...
		it('should be able to combine filters, pagination, ordering and select', async () => {
			for (let i = 1; i <= 10; i++) {
				await request(server)
					.post('/users')
					.set('Authorization', `Bearer ${env.MASTER_LOCAL_TESTS_KEY}`)
